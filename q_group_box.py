...
class QVersionBox(QGroupBox):  # Окно и виджеты на нем
    ...
    __versions_dir_2 = r'\\192.168.0.1\Files\Digitrock'
    ...
    def fill_versions(self):
        if os.path.exists(self.__versions_dir):
            path = self.__versions_dir
        elif os.path.exists(self.__versions_dir_2):
            path = self.__versions_dir_2
        else:
            return
    ...
    def on_btn_click(self):
        print(subprocess.getoutput("where python"))
        with open(f"{os.path.join(os.getcwd(), self.__bat_path)}", 'r+') as file:
